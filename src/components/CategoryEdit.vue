...
export default {
  ...
  watch: {
    current(catId) {
      const { title, wasteCode, sourceMaterials, processName, recyclingType, precaution, transportationRequirements, emergency, additionalInfo, wasteIndex, limit } = this.categories.find(c => c.id === catId)
      this.title = title
  ...
  created() {
    const { id, title, wasteCode, processName, sourceMaterials, recyclingType, precaution, transportationRequirements, emergency, additionalInfo, wasteIndex, limit } = this.categories[0]
    this.current = id
    this.title = title
    this.wasteCode = wasteCode
    this.sourceMaterials = sourceMaterials
