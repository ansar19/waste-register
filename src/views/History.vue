<template>
  <div>
    ...
    <section v-else>
      ...
      <Paginate v-model="page" :page-count="pageCount" :click-handler="pageChangeHandler" :prev-text="'Back' | localize"
        :next-text="'Forward' | localize" :container-class="'pagination'" :page-class="'waves-effect'" />
    </section>
  </div>
</template>
...
export default {
  ...
  async mounted() {
    ...
    this.setup(categoires, transporters, utilizators)

    this.loading = false
  },
  methods: {
    setup(categoires, transporters, utilizators) {
        this.setupPagination(
          this.records.map(record => {
            return {
              ...
              transpoterName: transporters.find(t => t.id === record.transporterId)
                .title,
              typeText: record.type === 'income' ?
                localizeFilter('Income') : localizeFilter('Outcome'),
